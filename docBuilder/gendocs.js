...
var helpers_1 = require("typedoc/dist/lib/output/utils/resources/helpers");
var templates_1 = require("typedoc/dist/lib/output/utils/resources/templates");
var filename = './docs/template.hbs';
var source = fs.readFileSync(filename, 'utf-8');
var datasource = fs.readFileSync("./docs/data.json");
var helpers = new helpers_1.HelperStack();
var partials = new templates_1.PartialStack();
partials.addOrigin("theme", Path.join(__dirname, 'partials'), true);
partials.activate();
helpers.addOrigin("theme", Path.join(__dirname, 'helpers'), true);
helpers.activate();
Marked.setOptions({
        ...
        element["children"].forEach(function (childElement) {
            ...
            if (childElement.children) {
                for (var i = childElement.children.length - 1; i >= 0; i--) {
                    ...
                    childElement.children[i].signatures = te.signatures;
