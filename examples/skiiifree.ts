...
class BasicGame extends m.Game
{
    ...
    {
      new Player();
      // add a new tree every 20 frames
      this.newFiber().do(()=>{new TreeEnemy()}).wait(20);
      // add a new score thing every 40 frames
      this.newFiber().do(()=>{new ScoreThing()}).wait(40);
    }
}
...
// before we start the game, setup your configuration.
...
m.Game.runOnReady(()=>{
    g = new BasicGame();
  	g.enableLeaderboard(getLeaderboardURL());
});
